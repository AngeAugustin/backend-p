...
export { GEMINI_QUOTA_ERROR_CODE, GeminiKeysExhaustedError, isGeminiQuotaError, isGeminiRateLimitError } from "@/lib/gemini/errors";
export {
  getAvailableApiKeys,
  getGeminiApiKeys,
...
/** Returns client for the first available key. */
export function getGeminiClient() {
  const apiKey = getAvailableApiKeys()[0];

  if (!apiKey) {
    throw new Error("GEMINI_API_KEYS is not configured or all keys are exhausted");
  }
